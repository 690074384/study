...
public class OnlineChatServer extends WebSocketServer {
     ...
     * @param user 用户
     */
    private void senFhsms(String user) {
     ...
     * @param user 用户
     */
    private void goOut(String user) {
     ...
     * @param conn 用户
     */
    private void goOut(WebSocket conn, String type) {
        JSONObject result = new JSONObject();
     ...
     * @param conn 用户
     */
    private void getUserCount(WebSocket conn) {
        JSONObject result = new JSONObject();
     ...
     */
    private synchronized void onlineMaganger(int type, String user, WebSocket conn) {
        if (type == 1) {
            //判断用户是否在其它终端登录
     ...
     * @param user 用户
     */
    private void addUserToFhadmin(String user) {
        WebSocket conn = OnlineChatServerPool.getFhadmin();
        if (null == conn) {
            return;
        }
